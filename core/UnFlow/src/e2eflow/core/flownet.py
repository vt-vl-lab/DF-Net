...
# (Yuliang) Add reuse_variables for forward flow
def flownet(im1, im2, flownet_spec='S', full_resolution=False, train_all=False,
            backward_flow=False, reuse=False):
    ...
    for i, name in enumerate(flownet_spec):
        ...
        def scoped_block(reuse=False):
            if name.lower() == 'c':
                assert i == 0, 'FlowNetS must be used for refinement networks'

                with tf.variable_scope('flownet_c_features') as scope:
                    # (Yuliang)
                    if reuse:
