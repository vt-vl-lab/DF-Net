...
# (Yuliang) Inverse warp with flow
def flow_inverse_warp(img, flow):
  batch, height, width, _ = img.get_shape().as_list()
  x_base = tf.range(width)
  y_base = tf.range(height)
  x_base = tf.stack([x_base]*height, axis=0)
  y_base = tf.transpose(tf.stack([y_base]*width, axis=0))
  flow0 = flow[:, :, :, 0]
  flow1 = flow[:, :, :, 1]
  flow0 = flow0 + tf.cast(x_base, tf.float32)
  flow1 = flow1 + tf.cast(y_base, tf.float32)
  coords = tf.stack([flow0, flow1], axis=-1)
  output_img = bilinear_sampler(img, coords)
  return output_img

def depth_pose_inverse_warp(img, depth, pose, intrinsics):
  batch, height, width, _ = img.get_shape().as_list()
  pixel_coords = meshgrid(batch, height, width)
  cam_coords = pixel2cam(depth, pixel_coords, intrinsics)
  filler = tf.constant([0.0, 0.0, 0.0, 1.0], shape=[1, 1, 4])
