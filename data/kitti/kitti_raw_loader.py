...
class kitti_raw_loader(object):
    def __init__(self, 
        ...
        excluded_frames_file = dir_path + '/excluded_frames.txt'
    ...
    def collect_excluded_frames(self, excluded_frames_file):
        ...
        for fr in frames:
            ...
            curr_fid = '%.10d' % (np.int(frame_id[:-1]))
            for cid in self.cam_ids:
                self.excluded_frames.append(drive + ' ' + cid + ' ' + curr_fid)
    ...
    def is_valid_sample(self, frames, tgt_idx):
        ...
        if self.seq_length % 2 != 0:
            ...
            left_offset = int(self.seq_length / 2 - 1)
            right_offset = int(self.seq_length / 2)
            min_src_idx = tgt_idx - left_offset
            max_src_idx = tgt_idx + right_offset
            if min_src_idx < 0 or max_src_idx >= N:
                return False
            min_src_drive, min_src_cid, _ = frames[min_src_idx].split(' ')
            max_src_drive, max_src_cid, _ = frames[max_src_idx].split(' ')
    ...
    def load_image_sequence(self, frames, tgt_idx, seq_length):
        if seq_length % 2 != 0:
            ...
            left_offset = int(seq_length / 2 - 1)
            right_offset = int(seq_length / 2)
            image_seq = []
            for o in range(-left_offset, right_offset + 1):
                curr_idx = tgt_idx + o
                curr_drive, curr_cid, curr_frame_id = frames[curr_idx].split(' ')
                curr_img = self.load_image_raw(curr_drive, curr_cid, curr_frame_id)
                if o == 0:
